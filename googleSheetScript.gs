...
function fetchApolloEmails() {
  ...
  values.forEach(function (row, rowIndex) {
    ...
    var hyperlinkCell = range.offset(
      rowIndex,
      hyperlinkCol - range.getColumn(),
      1,
      1
    );
    ...
    Logger.log(
      "Hyperlink formula in row " +
        (range.getRow() + rowIndex) +
        ": " +
        hyperlinkFormula
    );
    ...
    if (hyperlinkFormula.startsWith("=HYPERLINK")) {
      ...
      Logger.log(
        "Invalid HYPERLINK formula in row: " + (range.getRow() + rowIndex)
      );
    }
  });
}
...
function composeEmailDrafts() {
  ...
  values.forEach(function (row, rowIndex) {
    ...
    var fullName = range
      .offset(rowIndex, nameCol - range.getColumn(), 1, 1)
      .getValue();
    Logger.log("fullName: " + fullName);
    var toEmailAddress = range
      .offset(rowIndex, emailCol - range.getColumn(), 1, 1)
      .getValue();
    Logger.log("toEmailAddress: " + toEmailAddress);

    if (toEmailAddress != "Error fetching email" && toEmailAddress != "-") {
      var jobs = range
        .offset(rowIndex, jobsCol - range.getColumn(), 1, 1)
        .getValue();
      Logger.log("jobs: " + jobs);
      var companyName = range
        .offset(rowIndex, companyCol - range.getColumn(), 1, 1)
        .getValue();
      Logger.log("companyName: " + companyName);

      var resumeUrl =
        "https://firebasestorage.googleapis.com/v0/b/nikhil-nair.appspot.com/o/Latest%20Gen%20AI.pdf?alt=media&token=e16da10c-8e91-40a3-8d1a-5d5c5d836f7e";
      ...
      sendEmail(
        ...
        resumeBlob,
        function (status) {
          var emailedStatusCell = range.offset(
            rowIndex,
            emailedStatusCol - range.getColumn(),
            1,
            1
          );
          var emailReplyCell = range.offset(
            rowIndex,
            emailReplyStatusCol - range.getColumn(),
            1,
            1
          );
...
function sendEmail(
  ...
  var subject = `Application for ${jobStr} ${
    isPlural ? "Roles" : "Role"
  } at ${companyName}`;
  ...
  var body = `
    <div style="font-size:12.8px">
      Hi ${firstName},
      <br><br>
      Hope you're well! I came across your profile via a listing on LinkedIn for the ${jobStrHtml} ${
    isPlural ? "positions" : "position"
  } at ${companyName}.
      With 2 years of experience in strategy consulting and analytics, I'm eager to learn more about the ${
        isPlural ? "roles" : "role"
      }.
      <br><br>
      ...
      <br><br>
      I've attached my resume for your reference, which outlines my professional accomplishments in more detail. I'd be thrilled to discuss how my experience and skills align with the needs of the ${
        isPlural ? "roles" : "role"
      } at ${companyName}.
      <br><br>
  ...
  try {
    GmailApp.createDraft(toEmailAddress, subject, "", {
      htmlBody: body,
...
function updateStatus() {
  ...
  var batchSize = 5; // Number of tabs to open at once
  var batchCount = Math.ceil(urlList.length / batchSize);

  for (var i = 0; i < batchCount; i++) {
    var batchUrls = urlList.slice(i * batchSize, (i + 1) * batchSize);
    ...
    batchUrls.forEach(function (url) {
      ...
      Logger.log("Opened URL in modal dialog: " + url);
    });

    // Sleep for a short period before opening the next batch
